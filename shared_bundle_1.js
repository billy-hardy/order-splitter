 ...
 */function formatUSD(number){if(typeof number!=='number'){return;}return number.toLocaleString('en',{style:'currency',currency:'USD',useGrouping:true});}var main={formatUSD:formatUSD};class Order{static split(config){return new Order(config).split();}constructor(config={tip:0,tax:0,untaxedFees:0,taxedFees:0,isTipPercentage:false,people:{}}){if(config.taxedFees){console.warn('Taxed fees have not been tested and probably don\'t work. I was contemplating removing this feature.');}// validation
{var defaults={'isTipPercentage':false,'untaxedFees':0,'people':{},'tax':0,'taxedFees':0,'tip':0};var typeValidationMap={'isTipPercentage':'boolean','untaxedFees':'number','people':'object','tax':'number','taxedFees':'number','tip':'number'};Object.entries(defaults).forEach(function([key,default0]){config[key]=config[key]||default0;});Object.entries(config).forEach(function([key,value]){var expectedType=typeValidationMap[key];if(!expectedType){throw new Error(`Unexpected key ${key}`);}if(typeof value!==expectedType){throw new Error(`config.${key} must be of type ${expectedType}`);}delete typeValidationMap[key];});Object.keys(typeValidationMap).forEach(key=>{throw new Error(`Missing key ${key}`);});Object.values(config.people).forEach(personCost=>{if(typeof personCost!=='number'||isNaN(personCost)){throw new Error('config.people\'s values must be valid numbers');}});}// convert object to Map
config.people=Object.entries(config.people).reduce((map,[key,value])=>map.set(key,value),new Map());Object.assign(this,config);}withTip(tip,asPercentage=false){console.warn('withTip() is deprecated');this.isTipPercentage=asPercentage;this.tip=tip;this.isTipPercentage=asPercentage;return this;}withNonTaxedFees(...fees){console.warn("withNonTaxedFees() is deprecated");this.untaxedFees=fees.reduce((acc,val)=>acc+val);return this;}withTaxedFees(...fees){console.warn("withTaxedFees() is deprecated");this.taxedFees=fees.reduce((acc,val)=>acc+val);return this;}withTax(tax){console.warn("withTax() is deprecated");this.tax=tax;return this;}withPerson(name,price){console.warn("withPerson() is deprecated");let newPrice=price;if(this.people.has(name)){newPrice+=this.people.get(name);}this.people.set(name,newPrice);return this;}get taxPercent(){if(this.subTotal===0){return 0;}return this.tax/this.subTotal;}get taxPercentDisplay(){return this.taxPercent*100;}get fee(){return this.untaxedFees;}get tipPercent(){if(this.isTipPercentage){return this.tip;}else{if(this.subTotal===0){return 0;}return this.tipDollars/this.subTotal;}}get tipDollars(){if(this.isTipPercentage){return this.tipPercent*this.subTotal;}else{return this.tip;}}get feesPerPerson(){if(!this.hasPeople)return{};var subtotal=Array.from(this.people.values()).reduce((a,b)=>a+b);if(subtotal==0){// simply convert this.people from Map to standard js object
return Array.from(this.people).reduce((obj,[key,value])=>{obj[key]=value;return obj;},{});}return Array.from(this.people.entries()).reduce((feesPerPerson,[name,price])=>{feesPerPerson[name]=price/subtotal*this.untaxedFees;return feesPerPerson;},{});}get total(){return this.subTotal+this.fee+this.tipDollars+this.tax;}get hasPeople(){return!!Array.from(this.people.values()).length;}split(){this.totals=new Map();if(!this.hasPeople){this.subTotal,this.totalPrice=0;return this;}this.subTotal=Array.from(this.people.values()).reduce((sum,value)=>sum+value);this.subTotal+=this.taxedFees;for(let[name,price]of this.people.entries()){let totalForPerson=price;totalForPerson+=price*this.taxPercent;totalForPerson+=price*this.tipPercent;totalForPerson+=this.feesPerPerson[name];this.totals.set(name,totalForPerson);}let totalPrice=Array.from(this.totals.values()).reduce((acc,val)=>acc+val);if(totalPrice!=0&&Math.round(totalPrice*100)!=Math.round(this.total*100)){throw new Error('Everyone\'s share does not add up to total');}return this;}// not used. we should delete this
toJSON(){let ret={};ret.people=Array.from(this.people);ret.tipDollars=this.tipDollars;ret.tax=this.tax;ret.untaxedFees=this.untaxedFees;ret.taxedFees=this.taxedFees;ret.isTipPercentage=this.isTipPercentage;return ret;}// not used. we should delete this
static fromJSON(json){let order=new Order();order.people=new Map(json.people);order.withTip(json.tipDollars,false).withTax(json.tax).withNonTaxedFees(json.untaxedFees).withTaxedFees(json.taxedFees);return order.split();}}var order={Order:Order};class OrderUpHtmlParser{parse(containerElement){var highlight=element=>{element.style.background='repeating-linear-gradient(45deg, yellow, yellow 10px, #fafa0a 10px, #fafa0a 20px )';element.title='OrderSplitter is using this value';};var untaxedFees=0,tip,tax,people;var summaryTable=Array.from(containerElement.querySelectorAll('#order-confirmation-page tr'));if(summaryTable){[people]=summaryTable.map(tr=>Array.from(tr.querySelectorAll('td'))).filter(tds=>tds.length).reduce(function([people,lastItemCost],[left_td,center_td,right_td]){let name,price;if(right_td&&right_td.classList.contains('price-table')){price=right_td.innerText;highlight(right_td);lastItemCost=Number(price.match('\\$([0-9.]+)')[1]);}if(center_td&&(name=center_td.querySelector('li:last-child strong'))){highlight(name);name=name.innerText;people[name]=people[name]||0;people[name]+=lastItemCost;lastItemCost=null;}return[people,lastItemCost];},[{},null]);}var infoTable=Array.from(containerElement.querySelectorAll('div.order-information tr'));if(infoTable){infoTable.map(tr=>Array.from(tr.querySelectorAll('td'))).filter(tds=>tds.length).forEach(function([left,right]){let match=right.textContent.match('\\$([0-9.]+)');var value=match&&Number(match[1]);switch(left.textContent.trim()){case'Sales Tax':tax=value;highlight(right);break;case'Processing Fee':case'Delivery Fee':untaxedFees+=value;highlight(right);break;case'Tip':tip=value;highlight(right);break;}});}return Order.split({people,untaxedFees,tip,tax});}}class QueryStringParser{/**
   ...
   */parse(queryString){if(!location.search.length){return undefined;}var params=location.search.slice(1).split('&');console.debug('params',params);let people={},untaxedFees,tax,tip;params.map(param=>param.split('=')).forEach(function([key,value]){key=decodeURIComponent(key);value=Number(decodeURIComponent(value));switch(key){case'fee':untaxedFees=value;break;case'tax':tax=value;break;case'tip':tip=value;break;default:people[key]=value;}});return Order.split({people,untaxedFees,tax,tip});}}class OrderUpParser{/**
   ...
   */parse(elementOrText,fee=0,tax=0,tip=0,isTipPercentage=false){let orderUpText=elementOrText.innerText||elementOrText;let order=new Order().withNonTaxedFees(fee).withTax(tax).withTip(tip,isTipPercentage);var lines=orderUpText.split('\n');lines.reduce((lastItemCost,line)=>{let itemCostMatch,nameMatch;if(!lastItemCost){if(itemCostMatch=line.match('.*\\$([0-9.]+)')){let itemCost=Number(itemCostMatch[1]);return itemCost;}}if(nameMatch=line.match('.*Label for:(.*)')){let name=nameMatch[1];order.withPerson(name,lastItemCost);return;}return lastItemCost;},null);return order;}}class CsvParser{parse(element){let orderParams=element.innerText.split('\n').map(line=>line.trim()).filter(line=>!!line).reduce((orderParams,line)=>{let[name,...priceStrings]=line.split(',');let price=priceStrings.map(priceStr=>parseFloat(priceStr.replace('$',''))).filter(price=>!isNaN(price)).reduce((price,sum)=>price+sum,0);if(!price){// ignore names with price of $0
return orderParams;}switch(name){case'fee':orderParams.untaxedFees=orderParams.untaxedFees||0;orderParams.untaxedFees+=price;break;case'tax':orderParams.tax=orderParams.tax||0;orderParams.tax+=price;break;case'tip':orderParams.tip=orderParams.tip||0;orderParams.tip+=price;break;default:orderParams.people[name]=orderParams.people[name]||0;orderParams.people[name]+=price;}return orderParams;},{people:{}});return Order.split(orderParams);}}class GrubHubParser{parse(element){let order=new Order();let people=Array.from(element.querySelectorAll('[data-testid="receipt-item"]')).map(parent=>{return[parent.querySelector("[at-special-instruction]").innerText.replace(/\"/g,'').split("For ")[1],parsePrice(parent.lastChild.firstChild.innerText.replace(/\"/g,''))];});for(const person of people){order.withPerson(person[0],person[1]);}let receiptLineItems=new Map(Array.from(element.querySelectorAll('[at-receipt-line-item="true"]')).map(parent=>[parent.firstChild.innerText.replace(/\"/g,''),parsePrice(parent.lastChild.innerText.replace(/\"/g,''))]));order.withNonTaxedFees(-1*(receiptLineItems.get("Promo code:")||0));order.withTax(receiptLineItems.get("Sales tax:")||0);order.withTip(receiptLineItems.get("Tip:")||0);return order;}}function parsePrice(price){return Number(price.split("$")[1]);}function getUserInputParsers(){return[GrubHubParser,OrderUpHtmlParser,CsvParser];}var parsers={OrderUpHtmlParser:OrderUpHtmlParser,QueryStringParser:QueryStringParser,OrderUpParser:OrderUpParser,CsvParser:CsvParser,GrubHubParser:GrubHubParser,getUserInputParsers:getUserInputParsers};/**
   @license
...
if(!resolveDoc){resolveDoc=document.implementation.createHTMLDocument('temp');resolveDoc.base=resolveDoc.createElement('base');resolveDoc.head.appendChild(resolveDoc.base);resolveDoc.anchor=resolveDoc.createElement('a');resolveDoc.body.appendChild(resolveDoc.anchor);}resolveDoc.base.href=baseURI;resolveDoc.anchor.href=url;return resolveDoc.anchor.href||url;}/**
   ...
   */function pathFromUrl(url){return url.substring(0,url.lastIndexOf('/')+1);}var resolveUrl$1={resolveUrl:resolveUrl,resolveCss:resolveCss,pathFromUrl:pathFromUrl};const useShadow=!window.ShadyDOM||!window.ShadyDOM.inUse;const useNativeCSSProperties=Boolean(!window.ShadyCSS||window.ShadyCSS.nativeCss);const useNativeCustomElements=!window.customElements.polyfillWrapFlushCallback;const supportsAdoptingStyleSheets=useShadow&&'adoptedStyleSheets'in Document.prototype&&'replaceSync'in CSSStyleSheet.prototype&&// Since spec may change, feature detect exact API we need
(()=>{try{const sheet=new CSSStyleSheet();sheet.replaceSync('');const host=document.createElement('div');host.attachShadow({mode:'open'});host.shadowRoot.adoptedStyleSheets=[sheet];return host.shadowRoot.adoptedStyleSheets[0]===sheet;}catch(e){return false;}})();/**
    ...
    */const getSanitizeDOMValue=function(){return sanitizeDOMValue;};/**
 ...
 * @private
 */function runEffects(inst,effects,props,oldProps,hasPaths,extraArgs){if(effects){let ran=false;const id=dedupeId$1++;for(let prop in props){// Inline `runEffectsForProperty` for perf.
...
dedupeId$1++;const order=getComputedOrder(inst);const queue=[];for(let p in changedProps){enqueueEffectsFor(p,computeEffects,queue,order,hasPaths);}let info;while(info=queue.shift()){if(runComputedEffect(inst,'',changedProps,oldProps,info)){enqueueEffectsFor(info.methodInfo,computeEffects,queue,order,hasPaths);}}Object.assign(/** @type {!Object} */oldProps,inst.__dataOld);Object.assign(/** @type {!Object} */changedProps,inst.__dataPending);inst.__dataPending=null;}else{// Original Polymer 2.x computed effects order, which continues running
...
const cmp=order.get(queue[mid].methodInfo)-order.get(info.methodInfo);if(cmp<0){start=mid+1;}else if(cmp>0){end=mid-1;}else{idx=mid;break;}}if(idx<0){idx=end+1;}queue.splice(idx,0,info);};/**
    ...
    */const enqueueEffectsFor=(prop,computeEffects,queue,order,hasPaths)=>{const rootProperty=hasPaths?root(prop):prop;const fxs=computeEffects[rootProperty];if(fxs){for(let i=0;i<fxs.length;i++){const fx=fxs[i];if(fx.info.lastRun!==dedupeId$1&&(!hasPaths||pathMatchesTrigger(prop,fx.trigger))){fx.info.lastRun=dedupeId$1;insertEffect(fx.info,queue,order);}}}};/**
...
let nodes=templateInfo.childNodes;for(let i=0;i<nodes.length;i++){let node=nodes[i];wrap$1(wrap$1(node).parentNode).removeChild(node);}}/**
...
if(lastIndex&&lastIndex<text.length){let literal=text.substring(lastIndex);if(literal){parts.push({literal:literal});}}if(parts.length){return parts;}else{return null;}}/**
     ...
     *
     * @private
     */const hostStack=[];var propertyEffects={PropertyEffects:PropertyEffects};/**
   @license
   Copyright (c) 2017 The Polymer Project Authors. All rights reserved.
   This code may only be used under the BSD style license found at http://polymer.github.io/LICENSE.txt
   The complete set of authors may be found at http://polymer.github.io/AUTHORS.txt
...
return polymerElementBase._addTemplatePropertyEffect.call(this,templateInfo,prop,effect);}}return PolymerElement;});/**
     ...
     * @return {void}
     */const updateStyles=function(props){if(window.ShadyCSS){window.ShadyCSS.styleDocument(props);}};var elementMixin={version:version,builtCSS:builtCSS,ElementMixin:ElementMixin,updateStyles:updateStyles};class Debouncer{constructor(){this._asyncModule=null;this._callback=null;this._timer=null;}/**
...
/** @override */_initializeProperties(){if(this.hasAttribute(DISABLED_ATTR)){this.__isUpgradeDisabled=true;}else{super._initializeProperties();}}// Prevent element from enabling properties when it's upgrade disabled.
...
if(this.__isUpgradeDisabled&&value==null){super._initializeProperties();this.__isUpgradeDisabled=false;if(wrap$1(this).isConnected){super.connectedCallback();}}}else{super.attributeChangedCallback(name,old,value,/** @type {null|string} */namespace);}}// Prevent element from connecting when it's upgrade disabled.
...
/** @override */disconnectedCallback(){if(!this.__isUpgradeDisabled){super.disconnectedCallback();}}}return DisableUpgradeClass;});var disableUpgradeMixin={findObservedAttributesGetter:findObservedAttributesGetter,DisableUpgradeMixin:DisableUpgradeMixin};const bundledImportMeta$1={...import.meta,url:new URL('./node_modules/%40polymer/polymer/lib/legacy/legacy-element-mixin.js',import.meta.url).href};const DISABLED_ATTR$1='disable-upgrade';let styleInterface=window.ShadyCSS;/**
...
// provoke connectedCallback if the element is already connected.
if(this.__isUpgradeDisabled&&value==null){this._initializeProperties();this.__isUpgradeDisabled=false;if(wrap$1(this).isConnected){this.connectedCallback();}}}else{super.attributeChangedCallback(name,old,value,namespace);this.attributeChanged(name,old,value);}}}/**
...
if(legacyOptimizations&&this.hasAttribute(DISABLED_ATTR$1)){this.__isUpgradeDisabled=true;}else{let proto=Object.getPrototypeOf(this);if(!proto.hasOwnProperty(JSCompiler_renameProperty('__hasRegisterFinished',proto))){this._registered();// backstop in case the `_registered` implementation does not set this
...
}else if(n.localName==='slot'){if(hide){n.__polymerReplaced__=document.createComment('hidden-slot');wrap$1(wrap$1(n).parentNode).replaceChild(n.__polymerReplaced__,n);}else{const replace=n.__polymerReplaced__;if(replace){wrap$1(wrap$1(replace).parentNode).replaceChild(n,replace);}}}// hide and show nodes
...
let observer=new MutationObserver(()=>{if(wrap$1(this).querySelector('template')){observer.disconnect();this.__render();}else{throw new Error('dom-if requires a <template> child');}});observer.observe(this,{childList:true});return false;}this.__template=template;}return true;}/**
...
let lastChild=wrap$1(this).previousSibling;if(lastChild!==children[children.length-1]){for(let i=0,n;i<children.length&&(n=children[i]);i++){wrap$1(parentNode).insertBefore(n,this);}}}}return true;}/**
     * Forces the element to render its content. Normally rendering is
     * asynchronous to a provoking change. This is done for efficiency so
     * that multiple changes trigger only a single render. The render method
     * should be called if, for example, template rendering is required to
...
customStyleInterface$1.processStyles();},/**
   ...
   * @protected
   * @override
   */_shouldPropertyChange(property,value,old){return mutablePropertyChange$1(this,property,value,old,this.mutableData);}};var mutableDataBehavior={MutableDataBehavior:MutableDataBehavior,OptionalMutableDataBehavior:OptionalMutableDataBehavior};const Base=LegacyElementMixin(HTMLElement).prototype;var polymerLegacy={Base:Base,Polymer:Polymer,html:html};Polymer({is:'iron-location',_template:null,properties:{/**
...
// in a new tab.
if(event.metaKey||event.ctrlKey||event.shiftKey){return null;}var eventPath=dom(event).path;var anchor=null;for(var i=0;i<eventPath.length;i++){var element=eventPath[i];if(element.tagName==='A'&&element.href){anchor=element;break;}}// If there's no link there's nothing to do.
